...
// Seed source: db/seeds/*.json (ships with image, NOT symlinked to volume)
const path = require('path');
const { initSchema, ensureAdmin, ensureSettings, getDb } = require('./schema');

function seedIfEmpty() {
  const db = getDb();
  const hasData = (table) => db.prepare(`SELECT COUNT(*) as c FROM ${table}`).get().c > 0;

  // --- Products ---
  if (!hasData('products')) {
    const products = require(path.join(__dirname, 'seeds', 'products.json'));
    ...
    const tx = db.transaction(() => {
      products.forEach(p => {
        insertProduct.run(
          ...
          JSON.stringify(p.images || []), p.video || '',
          p.leadTime || '15-25 days', JSON.stringify(p.certifications || [])
        );
      });
    });
    tx();
  ...
  // --- Articles ---
  if (!hasData('articles')) {
    const articles = require(path.join(__dirname, 'seeds', 'blog.json'));
    ...
    const tx2 = db.transaction(() => {
      articles.forEach(a => {
        insertArticle.run(
          ...
          JSON.stringify(a.tags || []), a.image || '', a.video || '',
          a.excerpt || '', a.content || ''
        );
      });
    });
  ...
  // --- Categories (INSERT OR IGNORE = safe to run even if data exists) ---
  const categories = [
...
// Run standalone if invoked directly (node db/seed.js)
